...
public class MysticalCardGame {
    ...
    public static void startDuel(SLL<Card> firstSorcererCards, SLL<Card> secondSorcererCards) {
        ...
        int []firstimportants = new int[firstSorcererCards.size()];//za najgolem br

        SLLNode<Card> currentCardNode = firstSorcererCards.getFirst();// currant pokazuva kon prvata karta

        Card currentcard=currentCardNode.element;
        for (int i = 0; i < firstSorcererCards.size(); i++) {
                firstimportants[i]=currentcard.significance();
        }
        int bastcard=0;
        int index=0;
        for (int i = 0; i < firstSorcererCards.size(); i++) {
            if(bastcard<firstimportants[i]){
                bastcard=firstimportants[i];
                index=i;
            }
        }int br=0;
        SLLNode<Card> pred=firstSorcererCards.getFirst();
        SLLNode<Card> posle=pred;
        for (int i = 0; i <8; i++) {
            br++;
            if(br==4){
            }
